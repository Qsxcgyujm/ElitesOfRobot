...
import ElitesOfRobot.modules.sql.antispam_sql as gban_sql
import ElitesOfRobot.modules.sql.users_sql as user_sql
from ElitesOfRobot import dispatcher, OWNER_ID
...
def dbcleanup(update, context):
    ...
    invalidchats = msg.reply_text("Getting Invalid Chat Count...")
    invalid_chat_count = get_invalid_chats(context.bot, update)
    invalidchats.delete()

    invalidgbans = msg.reply_text("Getting Invalid Gbanned Count...")
...
LEAVE_MUTED_CHATS_HANDLER = CommandHandler(
    "getmc", leave_muted_chats, filters=Filters.chat(OWNER_ID)
)
